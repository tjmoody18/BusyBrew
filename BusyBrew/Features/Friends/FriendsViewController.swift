...
//    profileVC.userId = friend.uid // pass along their UID
...
class FriendsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func fetchFriends() {
        let uidToFetch = userId ?? currentUserUID
        db.collection("users").document(uidToFetch).getDocument { document, error in
            if let document = document, document.exists,
               let friendUIDs = document.data()?["friends"] as? [String] {
                var fetchedFriends: [Friend] = []
                let group = DispatchGroup()
    ...
    func removeFriend(_ friend: Friend) {
        ...
        alert.addAction(UIAlertAction(title: "Cancel", style: .cancel))
        alert.addAction(UIAlertAction(title: "Remove", style: .destructive, handler: { _ in
            let uidToFetch = self.userId ?? self.currentUserUID
            self.db.collection("users").document(uidToFetch).updateData([
                "friends": FieldValue.arrayRemove([friend.uid])
            ])
            self.db.collection("users").document(friend.uid).updateData([
                "friends": FieldValue.arrayRemove([uidToFetch])
            ])
            self.friendsList.removeAll { $0.uid == friend.uid }
            self.friendsTable.reloadData()
            self.showToast(message: "\(friend.name) removed.")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = friendsTable.dequeueReusableCell(withIdentifier: friendsCellIdentifier, for: indexPath) as! FriendsTableViewCell
        let friend = friendsList[indexPath.row]
        cell.profilePicture.image = UIImage(named: "cafe.png")
        cell.nameLabel.text = friend.name
        return cell
